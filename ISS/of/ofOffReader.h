...
class ofOffReader : public ofReader<_Traits>
{
public:
...
template <class _Traits>
ofOffReader<_Traits>::ofOffReader()
{}
...
template <class _Traits>
bool ofOffReader<_Traits>::read(sMesh* malha, const char *filename)
{
    OF_ASSERT(filename);
    OF_ASSERT(malha);
...
template <class _Traits>
bool ofOffReader<_Traits>::reader(ifstream &pf, sMesh* malha)
{
    char auxstr[255];
    space coords[3];
