...
class ofMesh2D : public ofMeshOper<_Traits>
{
public:
...
template <class _Traits>
ofMesh2D<_Traits>::ofMesh2D(sMesh *_mesh)
...
template <class _Traits>
ofMesh2D<_Traits>::~ofMesh2D()
{}

template <class _Traits>
void ofMesh2D<_Traits>::addCell(ids idcell, ids *idvertices OF_DMUTEXV)
{
    ...
    sVertex *v[3] =
        { 	this->mesh->getVertex(idv[0] OF_MUTEXV),
...
template <class _Traits>
void ofMesh2D<_Traits>::addCell(ids idcell, ids *idvertices, ids *idops OF_DMUTEXV)
{
    OF_ASSERT((idvertices[0] >= 0)&&(idvertices[1] >= 0)&&(idvertices[2] >= 0));
    OF_ASSERT((idvertices[0] != idvertices[1])&&(idvertices[0] != idvertices[2])&&(idvertices[1] != idvertices[2]));
    ...
    sVertex *v[3] =
        { 	this->mesh->getVertex(idvertices[0] OF_MUTEXV),
...
template <class _Traits>
void ofMesh2D<_Traits>::delCell(ids idcell OF_DMUTEXV)
{
    int i, pos;
...
template <class _Traits>
void ofMesh2D<_Traits>::lockVertices(ids *idv, sVertex** v)
{
    if(idv[0] < idv[1])
    {
        if(idv[0] < idv[2])
...
template <class _Traits>
void ofMesh2D<_Traits>::unlockVertices(ids *idv, sVertex** v)
{
    if(idv[0] < idv[1])
    {
        if(idv[0] < idv[2])
