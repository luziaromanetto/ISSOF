...
class ofSingularVertex : public ofVertex<size, _Traits>
{
public:
...
template <int size, class _Traits>
ofSingularVertex<size, _Traits>::ofSingularVertex()
{
#ifdef OF_THREADS
...
template <int size, class _Traits>
bool ofSingularVertex<size, _Traits>::isSingular(OF_DMUTEX)
{
    return (getNumberOfSings(OF_MUTEX) >= 2);
}

template <int size, class _Traits>
int ofSingularVertex<size, _Traits>::getNumberOfSings(OF_DMUTEX)
{
    OF_LOCK(smutex);
...
template <int size, class _Traits>
IDS ofSingularVertex<size, _Traits>::getSingCell(int pos OF_DMUTEXV)
{
    OF_ASSERT(pos >= 0);
...
template <int size, class _Traits>
bool ofSingularVertex<size, _Traits>::setSingCell(int pos, ids cell OF_DMUTEXV)
{
    OF_ASSERT(pos >= 0);
    OF_ASSERT(cell >= 0);
...
template <int size, class _Traits>
int ofSingularVertex<size, _Traits>::addSing(ids cell OF_DMUTEXV)
{
    OF_ASSERT(cell >= 0);
...
template <int size, class _Traits>
bool ofSingularVertex<size, _Traits>::delSing(int pos OF_DMUTEXV)
{
    OF_ASSERT(pos >= 0);
...
template <int size, class _Traits>
void ofSingularVertex<size, _Traits>::clearSings(OF_DMUTEX)
{
#ifdef OF_THREADS
    if(mutex)
        OF_LOCK(smutex);
...
template <int size, class _Traits>
SSING* ofSingularVertex<size, _Traits>::getSing(OF_DMUTEX)
{
    OF_LOCK(smutex);
...
template <int size, class _Traits>
int ofSingularVertex<size, _Traits>::inSings(ids cell OF_DMUTEXV)
{
    OF_LOCK(smutex);
...
//template <int size, class _Traits> void ofSingularVertex<size,_Traits>::addSing( sSing *s OF_DMUTEXD)
//{
//	OF_LOCK(smutex);
//
//	sSing *temp = sings;
