...
class ofOfWriter : public ofWriter<_Traits>
{
public:
...
template <class _Traits>
bool ofOfWriter<_Traits>::write(sMesh* malha, const char *filename)
{
    OF_ASSERT(filename);
    OF_ASSERT(malha);
...
template <class _Traits>
void ofOfWriter<_Traits>::writeVersion(ofstream &pf)
{
    pf.write("OF-BINARY-V1.001", 16*sizeof(char));
}

template <class _Traits>
void ofOfWriter<_Traits>::writeHeader(ofstream &pf, ids nv, ids nc)
{
...
template <class _Traits>
bool ofOfWriter<_Traits>::writer(ofstream &pf, sMesh* malha)
{
