...
class ofOfReader : public ofReader<_Traits>
{
public:
...
template <class _Traits>
bool ofOfReader<_Traits>::read(sMesh* malha, const char *filename)
{
    OF_ASSERT(filename);
    OF_ASSERT(malha);
...
template <class _Traits>
bool ofOfReader<_Traits>::checkVersion(ifstream &pf)
{
    char c;

    pf.read(&c, sizeof(char));
    if(c != 'O')
        return false;
    pf.read(&c, sizeof(char));
    if(c != 'F')
...
template <class _Traits>
bool ofOfReader<_Traits>::readHeader(ifstream &pf, ids &nv, ids &nc)
{
...
template <class _Traits>
bool ofOfReader<_Traits>::reader(ifstream &pf, sMesh* malha)
{
