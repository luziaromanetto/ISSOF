...
public:
    ...
    static space dist(sVertex *p0, sVertex *p1 OF_DMUTEXVD);
    static space dist(space *p0, space *p1 OF_DMUTEXV);
    ...
    bool isBadCell(sCell *c, space beta, space& c_beta OF_DMUTEXVD);
    ...
    static space dot(space *coords1, space *coords2, space *coords);
    space vecAngle(ids p1, ids p_ang, ids p2);
    static space norm2d(space *coords);
    static space areaTriangle(space *x, space *y, space *z);
...
template <class _Traits>
ofGeometric<_Traits>::ofGeometric(sMesh *_mesh, double _erro)
{
    mesh = _mesh;
    temp = 0;
    erro = _erro;
...
template <class _Traits>
ofGeometric<_Traits>::~ofGeometric()
{}

template <class _Traits>
void ofGeometric<_Traits>::setMesh(sMesh *_mesh)
{
    mesh = _mesh;
}
...
template <class _Traits>
SPACE ofGeometric<_Traits>::dist(sVertex *p0, sVertex *p1 OF_DMUTEXV)
{
...
template <class _Traits>
SPACE ofGeometric<_Traits>::dist(space *p0, space *p1 OF_DMUTEXV)
{
...
template <class _Traits>
int ofGeometric<_Traits>::inLeft(sVertex *p0, sVertex *p1, sVertex *p2 OF_DMUTEXV)
{
    ...
    //	space n_v, n_w;
    space v[2] =
        {p1->getCoord(0) - p0->getCoord(0), p1->getCoord(1) - p0->getCoord(1)};
    space w[2] =
        {p2->getCoord(0) - p0->getCoord(0), p2->getCoord(1) - p0->getCoord(1)};
...
template <class _Traits>
int ofGeometric<_Traits>::inLeft(sVertex *p0, sVertex *p1, space coord0, space coord1 OF_DMUTEXV)
{
    // Retorna 1 caso esteja a esquerda
    // Retorna 0 caso esteja a direita
    // Retorna 2 caso esteja sobre
...
template <class _Traits>
int ofGeometric<_Traits>::inLeft(ids p0, ids p1, ids p2 OF_DMUTEXV)
{
    // Retorna 1 caso esteja a esquerda
    // Retorna 0 caso esteja a direita
    // Retorna 2 caso esteja sobre
...
template <class _Traits>
int ofGeometric<_Traits>::inLeft(ids p0, ids p1, space coord0, space coord1 OF_DMUTEXV)
{
    // Retorna 1 caso esteja a esquerda
    // Retorna 0 caso esteja a direita
    // Retorna 2 caso esteja sobre
...
template <class _Traits>
int ofGeometric<_Traits>::inCircle(sVertex *p1, sVertex *p2, sVertex *p3, sVertex *p)
{

    space Bxmin, Bxmax, Bymin, Bymax, maxdiff;

    Bxmin = Bxmax = p->getCoord(0);
    Bymin = Bymax = p->getCoord(1);

    if( p1->getCoord(0) > Bxmax )
        Bxmax = p1->getCoord(0);
    if( p1->getCoord(0) < Bxmin )
        Bxmin = p1->getCoord(0);
    if( p1->getCoord(1) > Bymax )
        Bymax = p1->getCoord(1);
    if( p1->getCoord(1) < Bymin )
        Bymin = p1->getCoord(1);

    if( p2->getCoord(0) > Bxmax )
        Bxmax = p2->getCoord(0);
    if( p2->getCoord(0) < Bxmin )
        Bxmin = p2->getCoord(0);
    if( p2->getCoord(1) > Bymax )
        Bymax = p2->getCoord(1);
    if( p2->getCoord(1) < Bymin )
        Bymin = p2->getCoord(1);

    if( p3->getCoord(0) > Bxmax )
        Bxmax = p3->getCoord(0);
    if( p3->getCoord(0) < Bxmin )
        Bxmin = p3->getCoord(0);
    if( p3->getCoord(1) > Bymax )
        Bymax = p3->getCoord(1);
    if( p3->getCoord(1) < Bymin )
        Bymin = p3->getCoord(1);
    ...
    space v1[2], v2[2], v3[2], v[2];

    v1[0] = (p1->getCoord(0) - Bxmin)/maxdiff;
    v1[1] = (p1->getCoord(1) - Bymin)/maxdiff;

    v2[0] = (p2->getCoord(0) - Bxmin)/maxdiff;
    v2[1] = (p2->getCoord(1) - Bymin)/maxdiff;

    v3[0] = (p3->getCoord(0) - Bxmin)/maxdiff;
    v3[1] = (p3->getCoord(1) - Bymin)/maxdiff;

    v[0] = (p->getCoord(0) - Bxmin)/maxdiff;
    v[1] = (p->getCoord(1) - Bymin)/maxdiff;
...
template <class _Traits>
int ofGeometric<_Traits>::inCircle(sCell *c, sVertex *p OF_DMUTEXV)
{
    // Retorna 1 caso p esteja dentro do circuncirculo
    // Retorna 0 caso p esteja fora do circuncirculo
    // Retorna 2 caso p esteja sobre o circuncirculo
...
template <class _Traits>
SPACE ofGeometric<_Traits>::det(space matrix[4][4])
{
    return(
              (matrix[0][0]*(
...
template <class _Traits>
bool ofGeometric<_Traits>::inDiametralCircle(sVertex *v1, sVertex *v2, sVertex *v OF_DMUTEXV)
{
    /*
...
template <class _Traits>
bool ofGeometric<_Traits>::sameVertices(sVertex *v1, sVertex *v2 OF_DMUTEXV)
{
    return (fabs(v1->getCoord(0) - v2->getCoord(0)) < erro ) && ( fabs(v1->getCoord(1) - v2->getCoord(1)) < erro );
}

template <class _Traits>
int ofGeometric<_Traits>::orientation2D(ids *idvertices, sVertex **vertices OF_DMUTEXV)
{
...
template <class _Traits>
int ofGeometric<_Traits>::orientation3D(ids *idvertices, sVertex **vertices OF_DMUTEXV)
{
...
template <class _Traits>
bool ofGeometric<_Traits>::isBadCell(sCell *c, space beta, space& c_beta OF_DMUTEXV)
{
...
template <class _Traits>
void ofGeometric<_Traits>::getCircuncircle(space *c0, space *c1, space *c2, space *coords OF_DMUTEXV)
{
...
template <class _Traits>
void ofGeometric<_Traits>::getCircuncircle(sCell *c, space *coords OF_DMUTEXV)
{
...
template <class _Traits>
SPACE ofGeometric<_Traits>::det3(space matrix[3][3])
{
...
template <class _Traits>
SPACE ofGeometric<_Traits>::dot(ids p1, ids p2, space *coords OF_DMUTEXV)
{
...
template <class _Traits>
SPACE ofGeometric<_Traits>::dot(space *coords1, space *coords2, space *coords)
{
    //	sVertex *v1 = mesh->getVertex(p1), *v2 = mesh->getVertex(p2);
    double vv1X, vv1Y, vv2X, vv2Y, dp;

    vv1X = coords1[0] - coords[0];
    vv1Y = coords1[1] - coords[1];
    vv2X = coords2[0] - coords[0];
    vv2Y = coords2[1] - coords[1];
...
// este metodo calcula o angulo (em radianos) entre os vetores 2D: (p1-p_ang) e (p2-p_ang)
template <class _Traits>
SPACE ofGeometric<_Traits>::vecAngle(ids p1, ids p_ang, ids p2)
{
    sVertex *v1 = mesh->getVertex(p1),
                  *v2 = mesh->getVertex(p2),
                        *v_ang = mesh->getVertex(p_ang);
    double vv1X, vv1Y, vv2X, vv2Y, dp, aux, ang;
    ...
    ang = acos(dp/aux); // ang ?? um valor entre 0 e PI
    return ang;

}

template <class _Traits>
SPACE ofGeometric<_Traits>::norm2d(space *coords)
{
    return sqrt(coords[0]*coords[0] + coords[1]*coords[1]);
}

template <class _Traits>
SPACE ofGeometric<_Traits>::areaTriangle(space *p1, space *p2, space *p3)
{
    //	return fabs( (p1[0]-p3[0])*(p2[1]-p3[1])-(p1[1]-p3[1])*(p2[0]-p3[0]) )/2.0;
    return ((p1[0]-p3[0])*(p2[1]-p3[1])-(p1[1]-p3[1])*(p2[0]-p3[0]))/2.0;
}

template <class _Traits>
SPACE ofGeometric<_Traits>::areaTriangle(sCell *c)
{
...
template <class _Traits>
bool ofGeometric<_Traits>::isDelaunay(ids idcell)
{
    //	sCell *c = mesh->getCell(idcell);
    ids mates[3] =
        {mesh->getCell(idcell)->getMateId(0), mesh->getCell(idcell)->getMateId(1), mesh->getCell(idcell)->getMateId(2)};
    sVertex *v;
