...
public:
    ...
    bool read(const char *arquivo)
    {
        FILE *pf;
        char *buffer, *p, sep[] = " <>\n";
        int status = 0, i, vtemp = 0, ctemp = 0;
        long lSize;
        bool ler;
        ...
        while(p != NULL)
        {
            ...
            else if(status == 1)
            {
                if(strcmp(p, "database") == 0)
                    status = 2;
                else if(strcmp(p, "/OF_XML") == 0)
                    status = 0;
            }
            else if(status == 2)
            {
                if(strcmp(p, "name") == 0)
                    ler = attrib(db, strtok(NULL, sep));
                else if(strcmp(p, "host") == 0)
                    ler = attrib(host, strtok(NULL, sep));
                else if(strcmp(p, "port") == 0)
                    ler = attrib(port, strtok(NULL, sep));
                else if(strcmp(p, "user") == 0)
                    ler = attrib(user, strtok(NULL, sep));
                else if(strcmp(p, "password") == 0)
                    ler = attrib(pass, strtok(NULL, sep));
            ...
            {
                if(strcmp(p, "name") == 0)
                    ler = attrib(vtable, strtok(NULL, sep));
                else if(strcmp(p, "id") == 0)
                    ler = attrib(vid, strtok(NULL, sep));
            ...
            {
                if(strcmp(p, "name") == 0)
                    ler = attrib(ctable, strtok(NULL, sep));
                else if(strcmp(p, "id") == 0)
                    ler = attrib(cid, strtok(NULL, sep));
                else if(strcmp(p, "dimension") == 0)
                {
    ...
    char* getVerticesIdField()
    {
        return vid;
    };
    int   getVerticesDimension()
    {
        return vdim;
    };
    char* getVerticesField(int dim)
    {
        return coords[dim];
    };
    char* getCellsIdField()
    {
        return cid;
    };
    int   getCellsDimension()
    {
        return cdim;
    };
    char* getCellsField(int dim)
    {
        return vertices[dim];
    };
