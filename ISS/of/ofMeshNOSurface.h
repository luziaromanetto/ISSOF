...
class ofMeshNOSurface : public ofMeshOper<_Traits>
{
public:
...
template <class _Traits>
ofMeshNOSurface<_Traits>::ofMeshNOSurface(sMesh *_mesh)
        : ofMeshOper<_Traits>(_mesh)
{}

template <class _Traits>
ofMeshNOSurface<_Traits>::~ofMeshNOSurface()
{}

template <class _Traits>
void ofMeshNOSurface<_Traits>::addCell(ids idcell, ids *idvertices OF_DMUTEXV)
{
    ...
    ids idv[3] =
        { idvertices[0], idvertices[1], idvertices[2]};
    sVertex *v[3] =
        { this->mesh->getVertex(idv[0]), this->mesh->getVertex(idv[1]), this->mesh->getVertex(idv[2])};
    ...
    else if(ns == 1)
    {
        ...
        searchFace(idcell, idv[(temp+1)%3]);

        v[temp]->addSing(idcell);

        if (c->getMateId(temp) < 0)
        {
            // Nenhuma face em comum foi encontrada
            v[(temp+1)%3]->addSing(idcell);
            v[(temp+2)%3]->addSing(idcell);
        }
    }
    else
    {
...
template <class _Traits>
void ofMeshNOSurface<_Traits>::delCell(ids idcell OF_DMUTEXV)
{
}

template <class _Traits>
void ofMeshNOSurface<_Traits>::searchFace(int idcell, int idvertex)
{
    ...
    while(!lista.empty())
    {
        ...
        if((c != idcell)&&(ig == 2))
            if(setMate(c, idcell, iq))
                setMate(idcell, c, iq);
    }
}

template <class _Traits>
int ofMeshNOSurface<_Traits>::sameVertices(int idcell1, int idcell2, int iq[])
{
    int i, j, v1, v2, nv = 0;
...
template <class _Traits>
bool ofMeshNOSurface<_Traits>::setMate(int idcell, int idcellmate, int iq[])
{
    for(int i=0; i<3; i++)
    {
...
template <class _Traits>
void ofMeshNOSurface<_Traits>::searchSings(int idvertex)
{
    ofList<ids> lista;
    ofList<ids> flags;
