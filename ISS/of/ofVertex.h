...
class ofVertex : public ofBase<_Traits>
{
public:
...
template <int size, class _Traits>
ofVertex<size, _Traits>::ofVertex()
{}
...
template <int size, class _Traits>
SPACE ofVertex<size, _Traits>::getCoord(int pos OF_DMUTEXV)
{
    space c;
...
template <int size, class _Traits>
SPACE* ofVertex<size, _Traits>::getCoords(OF_DMUTEX)
{
    space *c;
...
template <int size, class _Traits>
void ofVertex<size, _Traits>::setCoord(int pos, space value OF_DMUTEXV)
{
    OF_ASSERT((pos>=0) && (pos < size));
...
template <int size, class _Traits>
void ofVertex<size, _Traits>::setCoords(space *values OF_DMUTEXV)
{
    OF_LOCK(smutex);
