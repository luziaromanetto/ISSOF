 ...
 */
template <int size, class _Traits>
class ofCell : public ofBase<_Traits>
{
public:
    ...
    /** Retorna a celula oposta a um vertice de indice dado.
     *
     * 	\param index: position of mate cell
     */
    ids	getMateId(int index OF_DMUTEXVD);

    /**	Retorna o ponteiro do vertice de indice dado
     *
     * 	\param index: position of vertex
     */
    ids	getVertexId(int index OF_DMUTEXVD);

    /** Define a celula oposta ao vertice de indice dado.
     *
     * 	\param index: position of mate cell
     ...
     */
    static inline int getDimension()
    {
        return size-1;
    };
...
template <int size, class _Traits>
IDS ofCell<size, _Traits>::getMateId(int index OF_DMUTEXV)
{
...
template <int size, class _Traits>
IDS ofCell<size, _Traits>::getVertexId(int index OF_DMUTEXV)
{
    OF_ASSERT((index >= 0)&&(index < size));
...
template <int size, class _Traits>
void ofCell<size, _Traits>::setMateId(int index, ids cell OF_DMUTEXV)
{
    OF_ASSERT((index >= 0)&&(index < size));
...
template <int size, class _Traits>
void ofCell<size, _Traits>::setVertexId(int index, ids vertex OF_DMUTEXV)
{
    OF_ASSERT((index >= 0)&&(index < size));
...
template <int size, class _Traits>
IDS ofCell<size, _Traits>::getMateVertexId(ids vertex OF_DMUTEXV)
{
    for(int i = 0; i < size; i++)
...
template <int size, class _Traits>
IDS ofCell<size, _Traits>::getVertexMateId(ids cell OF_DMUTEXV)
{   /* percorre os vertices da celula  de chamada checando se um dele tem como 
     * 'vizinho' a celula passada como parametro, se sim retorna o identificador 
     * do vertice, sen„o retorna o identificador invalido */
    for(int i = 0; i < size; i++)
...
template <int size, class _Traits>
int ofCell<size, _Traits>::getMateIndex(ids cell OF_DMUTEXV)
{
    for(int i = 0; i < size; i++)
        if (getMateId(i OF_MUTEXV) == cell)
            return i;
...
template <int size, class _Traits>
void ofCell<size, _Traits>::clearMates(OF_DMUTEX)
{
    for(int i = 0; i < size; i++)
...
template <int size, class _Traits>
int ofCell<size, _Traits>::getVertexIndex(ids vertex OF_DMUTEXV)
{
    for(int i = 0; i < size; i++)
        if (getVertexId(i OF_MUTEXV) == vertex)
            return i;
