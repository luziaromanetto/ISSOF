...
public:
    ...
    ofVis2D(int winX = 1024, int winY = 1024);
    ~ofVis2D();
...
template <class _Traits>
ofVis2D<_Traits>::ofVis2D(int winX, int winY)
{
    OF_ASSERT(winX > 0);
    OF_ASSERT(winY > 0);
...
template <class _Traits>
ofVis2D<_Traits>::~ofVis2D()
{}

template <class _Traits>
void ofVis2D<_Traits>::view(sMesh *_mesh)
{
    OF_ASSERT(_mesh);
    mesh = _mesh;
