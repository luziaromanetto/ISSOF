...
class ofPolyDataReader : public ofReader<_Traits>
{
public:
...
template <class _Traits>
ofPolyDataReader<_Traits>::ofPolyDataReader()
...
template <class _Traits>
void ofPolyDataReader<_Traits>::checkPoints(bool value)
{
    cpoints = value;
}

template <class _Traits>
bool ofPolyDataReader<_Traits>::read(sMesh* malha, const char *filename)
{
    OF_ASSERT(filename);
    OF_ASSERT(malha);
...
template <class _Traits>
bool ofPolyDataReader<_Traits>::reader(ifstream &pf, sMesh* malha)
{
    char auxstr[255];
    space coords[3];
...
template <class _Traits>
int ofPolyDataReader<_Traits>::sameVertices(sCell *cell, ids *vtx)
{
    int i, j, v1, v2, nv = 0;
